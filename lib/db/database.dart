...
class PastilleroDataBaseProvider {
  ...
  Future<Database> getDatabaseInstanace(
      String pathA, String query, String query2) async {
...
// Muestra todos los recordatorios
  ...
  Future<Reminder> getReminderWithId(int id) async {
    final db = await database;
    var response =
        await db.query("Reminder", where: "idReminder = ?", whereArgs: [id]);
  ...
  addReminderToDatabase(Reminder reminder) async {
    final db = await database;
    var table =
        await db.rawQuery("SELECT MAX(idReminder)+1 as id FROM Reminder");
